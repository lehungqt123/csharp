...
namespace QuanLyQuanCafe
{
    public class ExcelExport
    {
        ...
        public const string T_LoaiThucDon = "LoaiThucDon";
        public const string T_NhanVien = "NhanVien";
        public const string T_NhaCungCap = "NhaCungCap";
        public const string T_QLNguoiDung = "QLNguoiDung";
        public const string T_ChiTietThucDon = "ChiTietThucDon";
        #endregion
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case T_BieuMau:
                    ...
                    arrByte = File.ReadAllBytes("LoaiThucDon.xls").ToArray();
                    break;
                ...
                case T_NhanVien:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("NhanVien.xls").ToArray();
                    break;
                case T_NhaCungCap:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("NhaCungCap.xls").ToArray();
                    break;
                case T_QLNguoiDung:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("NguoiDung.xls").ToArray();
                    break;
                case T_ChiTietThucDon:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("ChiTietThucDon.xls").ToArray();
                    break;
                    #endregion
            }
            // Get stream
        ...
        public bool ExportNhanVien(List<NhanVien> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            ...
            return OutSimpleReport(dataSource, replacer, "NhanVien", isPrintPreview, ref fileName);
        }
        public bool ExportLoaiThucDon(List<LoaiThucDon> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            for (int i = 1; i <= dataSource.Count; i++)
            {
                //dataSource[i - 1].MaLoai = i.ToString();
            }
            //string tongSo = dataSource.Sum(t => t.SoLuong).Value.ToString();
            // Create replacer
            Dictionary<string, string> replacer = new Dictionary<string, string>();
            ...
            return OutSimpleReport(dataSource, replacer, "LoaiThucDon", isPrintPreview, ref fileName);
        }
        public bool ExportThucDon(List<ChiTietThucDon> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, "ChiTietThucDon", isPrintPreview, ref fileName);
        }
        public bool ExportNhaCungCap(List<NhaCungCap> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, "NhaCungCap", isPrintPreview, ref fileName);
        }
        public bool ExportNguoiDung(List<QLNguoiDung> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            for (int i = 1; i <= dataSource.Count; i++)
            {
                //dataSource[i - 1].TenDangNhap = i.ToString();
            }
            //string tongSo = dataSource.Sum(t => t.SoLuong).Value.ToString();
            // Create replacer
            Dictionary<string, string> replacer = new Dictionary<string, string>();
            ...
            return OutSimpleReport(dataSource, replacer, "QLNguoiDung", isPrintPreview, ref fileName);
        }
        #endregion
    }
}
